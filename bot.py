...
def check_subscription(user_id):
    ...
    for channel in channels:
        try:
            res = requests.get(f"{BASE_URL}/getChatMember", params={"chat_id": channel, "user_id": user_id}).json()
    ...
    if button == "broadcast":
        ...
        if check_subscription(user_id):
            ...
            for ch in get_channels():
                try:
                    res = requests.get(f"{BASE_URL}/getChatMember", params={"chat_id": ch, "user_id": user_id}).json()
                    status = res.get("result", {}).get("status", "")
                    if status not in ["member", "administrator", "creator"]:
            ...
            if "result" in updates and updates["result"]:
                for update in updates["result"]:
                    ...
                    if "message" in update:
                        ...
                        if chat_id != OWNER_ID:
                            channels = get_channels()
                            if channels:  # Agar kanal mavjud bo'lsa
                                if not is_subscribed(chat_id):  # Agar hali tasdiqlanmagan bo'lsa
                                    if not check_subscription(chat_id):  # Agar hamma kanallarga obuna bo'lmagan bo'lsa
                                        ...
                                        # Agar obuna bo'lgan bo'lsa, lekin tasdiqlamagan bo'lsa
                        ...
                        if is_instagram_url(text):
                            ...
                            if channels and not is_subscribed(chat_id):
                                # Agar kanal mavjud va foydalanuvchi tasdiqlamagan bo'lsa

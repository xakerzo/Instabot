...
def check_subscription(user_id):
    ...
    for channel in channels:
        try:
            ...
            # Agar xatolik bo'lsa, faqat logga yozamiz
            ...
            if "result" in updates and updates["result"]:
                for update in updates["result"]:
                    ...
                    if "message" in update:
                        ...
                        if chat_id != OWNER_ID:
                            if get_channels() and not check_subscription(chat_id):
                                # Inline tugma bilan kanalga yo‘naltirish
                                buttons = []
                                for ch in get_channels():
                                    buttons.append([{"text": ch, "url": f"https://t.me/{ch.replace('@','')}"}])
                                reply_markup = {"inline_keyboard": buttons}
                                send_message(chat_id, "Iltimos, barcha majburiy kanallarga obuna bo‘ling!", reply_markup)
                                continue

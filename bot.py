...
def handle_owner_buttons(data):
    ...
    if button == "broadcast":
        ...
        send_message(chat_id, "Iltimos, video caption matnini yuboring (format: caption: matn)")
            ...
            if "result" in updates and updates["result"]:
                for update in updates["result"]:
                    ...
                    if "message" in update:
                        ...
                        if chat_id == OWNER_ID and text.startswith("del@"):
                            ch = text.replace("del@", "").strip()
                            delete_channel(ch)
                            send_message(chat_id, f"Kanal o‘chirildi: {ch}")
                            continue

...
def send_media(chat_id, media_type, media_url, caption=None):
    try:
        if media_type == "photo":
            url = f"{BASE_URL}/sendPhoto"
            data = {"chat_id": chat_id, "photo": media_url, "caption": caption or "", "parse_mode": "HTML"}
        elif media_type == "video":
            url = f"{BASE_URL}/sendVideo"
            data = {"chat_id": chat_id, "video": media_url, "caption": caption or "", "parse_mode": "HTML"}
        else:
            return
...
def handle_owner_buttons(data):
    ...
    button = data["data"]
    if button == "broadcast":
        send_message(chat_id, "Broadcast matn, video yoki rasm yuboring (URL bilan):")
            ...
            if "result" in updates and updates["result"]:
                for update in updates["result"]:
                    ...
                    if "message" in update:
                        ...
                        # Owner broadcast (matn yoki URL) – matn, video, rasm
                        if chat_id == OWNER_ID and text.startswith("broadcast:"):
                            btext = text.replace("broadcast:", "").strip()
                            for uid in get_users():
                                send_message(uid, btext)
